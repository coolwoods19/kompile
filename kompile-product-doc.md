...
**Manual Input**
- Paste raw text (article excerpts, YouTube notes, personal notes)
- Upload markdown or text files

**Not in MVP (future):**
- Direct API connectors to Google Drive, Apple Notes, YouTube transcript APIs
- Browser extension for real-time capture
- Automatic sync with AI platforms

### 4.2 Compilation

The core engine. An LLM reads all ingested raw sources and compiles them into a structured markdown wiki.
...
2. **Knowledge Map** — Hierarchical structure: Domains → Subtopics → Concepts. Auto-generated from the content, showing how knowledge is organized.

3. **Cross-references & Backlinks** — Every article links to related articles. Concepts that appear across multiple domains are flagged.

4. **Cross-source Insights** — Connections the user might not have noticed. "Your Claude conversation about CUDA and your GPT conversation about cloud lock-in are making the same argument about switching costs."

5. **Blind Spots** — Gaps in knowledge. "You've extensively researched inference costs but have nothing about training cost trends."

6. **Index file** — Lightweight summary of all articles (title, one-line summary, concepts, backlinks). This is what gets loaded into AI context for navigation — typically < 500 tokens for the entire knowledge base.

**Incremental compilation:** When new sources are added, the compiler doesn't rebuild from scratch. It reads the new sources + existing index, and updates/creates articles as needed.

**Compilation is the most token-intensive step.** Initial compilation of ~10 conversation exports might cost $1-3 in API calls. Incremental updates are much cheaper. This is a one-time cost, not per-query.
...
│   ├── index.md          # Master index (lightweight)
│   ├── articles/
│   ├── concepts/
│   └── insights.md
...
|---|---|---|
| `get_index` | All article titles + one-line summaries + concept tags | 300-500 tokens |
| `get_article(id)` | Full content of one article | 500-800 tokens |
| `get_insights` | Cross-source insights + blind spots | 200-300 tokens |
| `search(query)` | Matching paragraphs across articles | 200-500 tokens |
| `get_concepts` | All concepts with descriptions and related articles | 200-400 tokens |

**How a typical query flows:**

1. User asks Claude a question
2. Claude calls `get_index` (~400 tokens) — sees what's in the knowledge base
3. Claude decides which 2-3 articles are relevant
4. Claude calls `get_article` for each (~2000 tokens total)
5. Claude synthesizes answer from knowledge base + its own knowledge + web search if needed
6. Total knowledge base cost per query: ~2500 tokens out of 200K context window
